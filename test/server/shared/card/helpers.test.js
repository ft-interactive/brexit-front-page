...
import { objMap, responsiveClass, responsiveValue } from '../../../../shared/libs/helpers';

describe('Card helpers', () => {
	describe('#objMap', () => {
		it('Maps an object with a single key', () => {
			const result = objMap({a: 3}, (it) => it + 1);

			expect(result).to.eql({a: 4});
		});

		it('Maps an object with a multiple keys', () => {
			const result = objMap({a: 3, b: 2}, (it) => it + 1);

			expect(result).to.eql({a: 4, b: 3});
		});

		it('Maps provides the key to the callback', () => {
			const result = objMap({a: 3, b: 2}, (it, key) => key);
